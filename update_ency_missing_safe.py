...
get_display_name_additions = '''            if (name.Contains("SwitchPower_220V")) return "220V开关电源（待支持）";
            if (name.Contains("PLC_Output_24V")) return "PLC可编程控制器（待支持）";
            if (name.Contains("SolenoidValve_24V")) return "电磁阀（待支持）";
            if (name.Contains("StepperDriver_24V")) return "步进电机驱动器（待支持）";
            if (name.Contains("StepperMotor_24V")) return "步进电动机（待支持）";
...
# Add search aliases
search_aliases_additions = '''            else if (defName.Contains("SwitchPower")) aliases = "开关电源 稳压电源";
...
if 'else if (defName.Contains("Indicator_")) aliases = "信号灯 运行灯 报警灯";' in content:
    content = content.replace('else if (defName.Contains("Indicator_")) aliases = "信号灯 运行灯 报警灯";',
                              'else if (defName.Contains("Indicator_")) aliases = "信号灯 运行灯 报警灯";\n' + search_aliases_additions)
else:
    print("WARNING: Could not find Indicator_ block in search aliases!")
